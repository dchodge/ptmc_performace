...
RCPP_EXPOSED_CLASS(EvaluateLogLikelihood)
...
public:
    ...
    static NumericVector parameterValuesTransformed;
    ...
    public:
      ...
      ODE_desc(double dailyBirthRate_t, NumericMatrix contactMatrix_t): dailyBirthRate(dailyBirthRate_t), contactMatrix(contactMatrix_t){
        ga0 = parameterValuesTransformed["ga0"];
        ga1 = 1;
        om = parameterValuesTransformed["om"];
        a = parameterValuesTransformed["a"];
        b = parameterValuesTransformed["b"];
        psi = parameterValuesTransformed["psi"];
        phi = parameterValuesTransformed["phi"];
        d1 = parameterValuesTransformed["d1"];
        alpha = parameterValuesTransformed["alpha"];
      };
      ...
      void operator() (  num_vec &x , num_vec &dxdt , const double  t )
      {
        ...
        for (int a = 0; a < A ; a++)
        {
          ...
          I_t = 0.0;
          for (int b = 0; b < A ; b++){
            I_t += (x[7*b+1]+x[7*b+4]*alpha)/populationPerAgeGroup[b]*contactMatrix(b,a);
          }
    ...
    double evaluateLogLikelihoodCpp(VectorXd currentParamValues)
    {
        ...
        ODE_desc ODE_desc_inst(this->dailyBirthRate, this->contactMatrix);
    ...
    void transformParameterValuesforODE(VectorXd currentParamValues){
        this->parameterValuesTransformed = NumericVector::create(
                                                                 ...
                                                                 _["a"] = logisticTransform(currentParamValues(2), 0, 1),
        ...
        ep_t(this->A);
        this->ep_t << this->parameterValuesTransformed["ep1"], this->parameterValuesTransformed["ep2"];
    }
    ...
    double evaluateTimeStepLogLikelihood(int weekNo){
        ...
        for (int a = 0; a < this->A; a++){
            NumericVector d = this->observedData[a];
            double dataNewInfections = d[weekNo];
    ...
    inline void checkStability(num_vec x)
    {
        ...
        for (int j = 0; j < this->A*7; j++){
            if (x[j] < 0) {this->loglikelihoodError=true; }
            else {X_w += x[j]; }
        }

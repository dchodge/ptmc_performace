...
density = function(params){
  ...
  for (i in 1:13)
  {
    p1 = p1 + dlogis(params[i], location = 0, scale = 1, log = FALSE)
  }
return(p1)
}
...
post <- ptmc_func(model, data, settingsPT)
...
grid.arrange(p1, p2,  top="Adaptive Metropolis-Hastings")
...
l <- length(outPTlp[[1]])
lpPT <- data.frame(loglike=log(-as.vector(sapply(c(1:no_chains), function(x) outPTlp[[x]] ))))
lpPT$chain_no <- as.vector(sapply(c(1:no_chains), function(x) rep(toString(x), l)))
lpPT$sample_no <- rep(1:l,no_chains)
lpPT$mcmc <- rep("PT", l*no_chains)

lpPT_conv <- lpPT %>% filter(sample_no>round(l/2))

p1 <- ggplot(lpPT, aes(x = sample_no, y = loglike)) + 
  geom_line(aes(color = chain_no), size = 0.2, alpha=0.8) +
  theme_minimal()

p2 <- ggplot(lpPT_conv, aes(x = sample_no, y = loglike)) + 
  geom_line(aes(color = chain_no), size = 0.2, alpha=0.8) +
  theme_minimal()
...
lpAll <- bind_rows(lpMH, lpAM, lpDezs, lpPT)
...
```{r}
alt <- lpPT %>% filter(chain_no == 10)

ggplot(alt, aes(x = sample_no, y = loglike)) + 
  geom_line(aes(color = mcmc), size = 0.2, alpha=0.8) +
  theme_minimal()
